...
export function setupLogging(ctx: DaemonPluginContext) {
  const time = new Date();
  const stamp = `${time.getFullYear()}-${
    time.getMonth() + 1
  }-${time.getDate()}_${time.getHours()}-${time.getMinutes()}-${time.getSeconds()}`;
  const appLevels: Logger.LevelConfig = { base: Logger.INFO, sysinfo: Logger.SILENT };
  const files: Array<{ path: string; archive: string; levels: Logger.LevelConfig }> = [
    ...
    {
      path: "logs/sysinfo.log",
      archive: `logs/sysinfo_${stamp}.log`,
      levels: { base: Logger.SILENT, sysinfo: Logger.INFO }
    }
  ];
