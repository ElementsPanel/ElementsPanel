...
the routes, the page, the editor, the viewer, the upload queue and the dialogs all
...
| `normal/{FileManager,FileEditor,ImageViewer}.vue` | The file manager page and its two dialogs. |
| `desktop/Desktop*.vue` | The same three, as Desktop windows. |

The file manager renders directly as the `/instances/terminal/files` route.
...
| `plugins/console`'s appearance settings, `InstanceDetail.vue` | The "pick a file" buttons return nothing, so the image or URL is left unchanged. |
