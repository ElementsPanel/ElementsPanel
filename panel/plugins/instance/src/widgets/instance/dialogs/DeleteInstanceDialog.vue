...
const submit = async (deleteFile = deleteFiles.value) => {
  ...
  try {
    const result = await execute({
      ...
      data: { uuids: [props.instanceId || ""], deleteFile: Boolean(deleteFile) }
    });
    const error = getInstanceDeleteError(result.value);
