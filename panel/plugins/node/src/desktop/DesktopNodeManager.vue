...
import { testSocketConnection, SocketStatus } from "@/hooks/useSocketIo";
...
import { getValidatorErrorMsg, reportErrorMsg } from "@/tools/validator";
...
const socketChecks = new Map<string, AbortController>();
let disposed = false;
let latestFetch = 0;
...
const testNodeSocket = async (node: NodeStatus) => {
  if (disposed || socketChecks.has(node.uuid)) return;
  if (!node.available) {
    socketStatusMap[node.uuid] = SocketStatus.Error;
    return;
  }
  const controller = new AbortController();
  socketChecks.set(node.uuid, controller);
  try {
    const addr = `${node.ip}:${node.port}`;
    await testSocketConnection(addr, node.prefix, controller.signal);
    if (!disposed && !controller.signal.aborted)
      socketStatusMap[node.uuid] = SocketStatus.Connected;
  } catch {
    if (!disposed && !controller.signal.aborted) socketStatusMap[node.uuid] = SocketStatus.Error;
  } finally {
    if (socketChecks.get(node.uuid) === controller) socketChecks.delete(node.uuid);
  }
};

const testAllSockets = async () => {
  await Promise.all(nodes.value.map(testNodeSocket));
};
...
onUnmounted(() => {
  ...
  socketChecks.forEach((controller) => controller.abort());
  socketChecks.clear();
});
...
const fetchNodes = async (silent = false) => {
  ...
  try {
    const res = await (silent ? fetchNodesSilentApi : fetchNodesApi)({ forceRequest: !silent });
    if (disposed || request !== latestFetch || !res.value) return;
    for (const previous of nodes.value) {
      const current = res.value.find((node) => node.uuid === previous.uuid);
      if (
        !current ||
        ...
        socketChecks.get(previous.uuid)?.abort();
        socketChecks.delete(previous.uuid);
        delete socketStatusMap[previous.uuid];
      }
    }
    nodes.value = res.value;
    await testAllSockets();
  } catch (error) {
    if (!silent && !disposed && request === latestFetch) reportErrorMsg(error);
  }
};
...
const filteredNodes = computed(() => {
  ...
  return nodes.value.filter(
    (n) => n.remarks.toLowerCase().includes(q) || n.ip.toLowerCase().includes(q)
  );
});
...
const saveNode = async () => {
  ...
  if (!form.ip.trim()) {
    formError.value = t("TXT_CODE_DESKTOP_NODES_IP_REQUIRED");
    return;
  }

  if (!Number.isInteger(Number(form.port)) || Number(form.port) < 1 || Number(form.port) > 65535) {
    formError.value = t("TXT_CODE_12040bf0");
    return;
  }

  if (dialogMode.value === "add" && !form.apiKey.trim()) {
    formError.value = t("TXT_CODE_DESKTOP_NODES_APIKEY_REQUIRED");
    return;
  }
  ...
  try {
    ...
    formError.value = getValidatorErrorMsg(error, t("TXT_CODE_6a365d01"));
  } finally {
    saving.value = false;
  }
};
...
const executeDelete = async () => {
  if (!deletingNode.value || saving.value) return;
  saving.value = true;
  try {
...
const reconnectNode = async (node: NodeStatus) => {
  if (reconnecting.has(node.uuid)) return;
  reconnecting.add(node.uuid);
  try {
    await connectNodeApiExec({
      params: {
        uuid: node.uuid
  ...
  <div class="desktop-nodes">
    <div class="dn-toolbar">
      <div class="dn-search">
        <VTextField
          v-model="searchQuery"
          class="dn-search__input"
          :placeholder="t('TXT_CODE_DESKTOP_NODES_SEARCH')"
    ...
    >
      <template #item.status="{ item }"
        ><VIcon
          :icon="item.available ? 'mdi-check-circle-outline' : 'mdi-close-circle-outline'"
          :class="item.available ? 'dn-badge-icon--yes' : 'dn-badge-icon--no'"
      /></template>
      <template #item.socket="{ item }"
        ><VIcon
          v-if="socketStatusMap[item.uuid] === SocketStatus.Connected"
          icon="mdi-check-circle-outline"
          class="dn-badge-icon--yes"
        /><VTooltip
          v-else-if="socketStatusMap[item.uuid] === SocketStatus.Error"
          :text="t('TXT_CODE_6b4a27dd')"
          ><template #activator="{ props: tooltipProps }"
            ><span v-bind="tooltipProps" class="dn-badge-icon--no"
              ><VIcon icon="mdi-information-outline" /></span></template></VTooltip
        ><span v-else>--</span></template
      >
      <template #item.version="{ item }"
        ><VTooltip
          v-if="item.available && item.brand !== 'ElementsPanel'"
          :text="t('TXT_CODE_NODE_BRAND_ERR')"
          ><template #activator="{ props: tooltipProps }"
            ><span v-bind="tooltipProps" class="dn-badge-icon--warn"
              ><VIcon icon="mdi-information-outline" /></span></template></VTooltip
        ><VIcon
          v-else-if="
            item.available &&
            nodeVersionMap[item.uuid] &&
            !hasVersionUpdate(specifiedDaemonVersion, nodeVersionMap[item.uuid])
          "
          icon="mdi-check-circle-outline"
          class="dn-badge-icon--yes"
        /><span v-else>--</span></template
      ...
      <template #item.name="{ item }"
        ><div class="dn-node-name">
          <VIcon icon="mdi-server-network" class="dn-node-name__icon" /><span>{{
      ...
      <template #item.actions="{ item }"
        ><div class="dn-action-btns">
          <VBtn
            ...
            @click="reconnectNode(item)"
            ><VIcon icon="mdi-refresh" /></VBtn
          ><VBtn
            icon
            variant="text"
            rounded="xl"
            class="dn-action-btn"
            :title="t('TXT_CODE_DESKTOP_NODES_EDIT')"
            @click="openEditDialog(item)"
            ><VIcon icon="mdi-pencil-outline" /></VBtn
          ><VBtn
            icon
            variant="text"
            rounded="xl"
            ...
            @click="openAdvancedSettings(item)"
            ><VIcon icon="mdi-cog-outline" /></VBtn
          ><VBtn
            icon
            variant="text"
            rounded="xl"
      ...
      <Transition name="dn-dialog-fade">
        <DesktopWindow
          ...
          :title="
            dialogMode === 'add'
              ? t('TXT_CODE_DESKTOP_NODES_ADD')
              : t('TXT_CODE_DESKTOP_NODES_EDIT')
          "
          :icon="dialogMode === 'add' ? PlusOutlined : EditOutlined"
          :visible="showDialog"
          :minimized="false"
          :maximized="false"
          :active="true"
          :initial-width="420"
          :initial-height="485"
          :initial-x="windowWidth / 2 - 210"
          :initial-y="windowHeight / 2 - 230"
          :z-index="10001"
          :show-minimize="false"
          :show-maximize="false"
          :resizable="false"
            ...
            <div class="dn-dialog__body">
              ...
              <div class="dn-form-group">
                ...
                <VTextField
                  ...
                  :placeholder="
                    dialogMode === 'edit' ? t('TXT_CODE_DESKTOP_NODES_APIKEY_HELP') : ''
                  "
                />
    ...
    <Teleport to="body">
      <Transition name="dn-dialog-fade">
        <DesktopWindow
          ...
          :initial-width="400"
          :initial-height="200"
          :initial-x="windowWidth / 2 - 200"
          :initial-y="windowHeight / 2 - 100"
          :z-index="10002"
          :show-minimize="false"
          :show-maximize="false"
          :resizable="false"
              ...
              <p class="dn-dialog__desc">
                {{
                  t("TXT_CODE_DESKTOP_NODES_DELETE_CONFIRM", {
                    name: deletingNode?.remarks || deletingNode?.ip
                  })
                }}
              </p>
            </div>
    ...
    <Teleport to="body">
      <Transition name="dn-dialog-fade">
        <DesktopNodeAdvancedSettings
          v-if="showAdvancedSettings && advancedSettingsNode"
          :visible="showAdvancedSettings"
          :node="advancedSettingsNode"
